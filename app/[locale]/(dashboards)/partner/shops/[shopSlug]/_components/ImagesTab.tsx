...
import { setShopImageAsCover, reorderShopImages } from "@/lib/actions/partner-shops";
...
export function ImagesTab({ shopId, images: initialImages }: Props) {
  ...
  function handleSetCover(imageId: string) {
    ...
    start(async () => {
      const res = await setShopImageAsCover(imageId, shopId);
      if (res.success) {
        setImages((prev) => [
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        ...
        <ImageUploader
          ...
          images={images}
          onChange={setImages}
          onReorder={(orderedIds) => reorderShopImages(shopId, orderedIds)}
        />
      </div>
