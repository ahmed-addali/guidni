...
function GuidniReviewSection({
  ...
  const t       = useTranslations("PartnerDashboard.editShop.reviews");
  ...
  const status     = guidniReview
    ? {
        label: t(`guidni.statuses.${statusKey}` as Parameters<typeof t>[0]),
        color: STATUS_COLOR[statusKey] ?? STATUS_COLOR.PENDING,
      }
    : null;
        ...
        <div className="space-y-3 pt-1 border-t border-primary/10">
          {guidniReview.reviewerName && (
            <p className="text-xs text-gray-500">
              {t("guidni.by")}{" "}
              <span className="font-medium text-gray-700">{guidniReview.reviewerName}</span>
          ...
          {guidniReview.scoreTotal != null && (
            <p className="text-xs text-gray-500">
              ...
              <span className="font-semibold text-gray-800">
                {guidniReview.scoreTotal} {t("guidni.scoreMax")}
              </span>
            </p>
          )}
        </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-medium text-gray-800">
              {review.user.name ?? t("guest.guestFallback")}
            </p>
...
export function ReviewsTab({ reviews, guidniReview, shopId }: Props) {
  const t        = useTranslations("PartnerDashboard.editShop.reviews");
  ...
  return (
    <div className="space-y-6">
      <GuidniReviewSection guidniReview={guidniReview} shopId={shopId} />
          ...
          {avg && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-400">
                {reviews.length}{" "}
                {t(reviews.length !== 1 ? "guest.reviewPlural" : "guest.reviewSingular")}
              </span>
              {unanswered > 0 && (
