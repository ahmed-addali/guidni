...
export async function generateMetadata() {
  const t = await getTranslations("PartnerShops.wizard");
  return { title: `${t("newShopTitle")} · Partner Dashboard` };
}
...
export default async function NewShopPage({ params }: { params: Params }) {
  ...
  const [profile, destinations, t] = await Promise.all([
    ...
    getDestinations(),
    getTranslations("PartnerShops.wizard"),
  ]);
  ...
  return (
    <div className="space-y-6 max-w-screen-md">
      ...
      <div>
        <h1 className="text-2xl font-bold text-gray-900">{t("newShopTitle")}</h1>
        <p className="text-sm text-gray-400 mt-1">{t("newShopSubtitle")}</p>
      </div>
      <CreateShopWizard
