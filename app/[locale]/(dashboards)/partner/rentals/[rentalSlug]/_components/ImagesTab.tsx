...
import { setRentalImageAsCover, reorderRentalImages } from "@/lib/actions/partner-rentals";
...
export function ImagesTab({ rentalId, images: initialImages }: Props) {
  ...
  function handleSetCover(imageId: string) {
    ...
    start(async () => {
      const res = await setRentalImageAsCover(imageId, rentalId);
      if (res.success) {
        setImages((prev) => [
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-6 space-y-3">
        ...
        <ImageUploader
          ...
          images={images}
          onChange={setImages}
          onReorder={(orderedIds) => reorderRentalImages(rentalId, orderedIds)}
        />
      </div>
    </div>
  );
