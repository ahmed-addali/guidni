...
import { deleteRental, updateRentalStatus } from "@/lib/actions/partner-rentals";
import { ConfirmDialog } from "@/components/partner/ConfirmDialog";

type RentalStatus = "DRAFT" | "ACTIVE" | "SUSPENDED";
...
export function SettingsTab({ rentalId, rentalTitle, rentalStatus }: Props) {
  const router    = useRouter();
  const params    = useParams();
  const locale    = params.locale as string;
  const t         = useTranslations("PartnerDashboard.editRental.settings");
  const [, start] = useTransition();
  const [deleting, setDeleting]           = useState(false);
  const [statusPending, setStatusPending] = useState(false);
  const [currentStatus, setCurrentStatus] = useState<RentalStatus>(rentalStatus);

  function handleStatusToggle() {
    const nextStatus: RentalStatus = currentStatus === "ACTIVE" ? "DRAFT" : "ACTIVE";
    setStatusPending(true);
    start(async () => {
      const res = await updateRentalStatus(rentalId, nextStatus);
      if (res.success) {
  ...
  function handleDelete() {
    setDeleting(true);
    start(async () => {
      const res = await deleteRental(rentalId);
      if (res.success) {
