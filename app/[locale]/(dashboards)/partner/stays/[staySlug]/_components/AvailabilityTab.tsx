...
import { toggleBlockedDate } from "@/lib/actions/partner";
...
/** Expand a [checkIn, checkOut) range into YYYY-MM-DD strings (checkIn inclusive, checkOut exclusive). */
function expandDates(checkIn: string, checkOut: string): string[] {
  const start = parseISO(checkIn);
  const end   = parseISO(checkOut);
  if (start >= end) return [];
  // subtract 1 day from end so eachDayOfInterval is inclusive-inclusive
  const endInclusive = new Date(end.getTime() - 86_400_000);
  return eachDayOfInterval({ start, end: endInclusive }).map((d) =>
    format(d, "yyyy-MM-dd")
  );
}
...
export function AvailabilityTab({
  ...
  const t = useTranslations("PartnerDashboard.editStay.availability");
  ...
  const { confirmedSet, pendingSet } = useMemo(() => {
    ...
    for (const r of initialReservations) {
      const dates = expandDates(r.checkIn, r.checkOut);
  ...
  const handleToggle = useCallback(
    (day: Date) => {
      ...
      // Reservation dates are read-only; only manually block/unblock
      if (confirmedSet.has(dateStr) || pendingSet.has(dateStr)) return;
      if (toggling) return;
      setToggling(dateStr);
      start(async () => {
        const res = await toggleBlockedDate(stayId, dateStr);
        if (res.success) {
          setBlocked((prev) => {
    ...
    [stayId, today, toggling, confirmedSet, pendingSet, t]
  );
                ...
                {initialReservations.map((r) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4 text-xs text-gray-600 shrink-0">
                      <span>
                        <span className="text-gray-400 mr-1">{t("labelCheckIn")}</span>
                        {r.checkIn}
                      </span>
                      <span>
                        <span className="text-gray-400 mr-1">{t("labelCheckOut")}</span>
                        {r.checkOut}
                      </span>
                    </div>
                  </div>
