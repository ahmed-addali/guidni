...
import { deleteActivity, updateActivityStatus } from "@/lib/actions/partner";
...
export function SettingsTab({
  ...
  const t                = useTranslations("PartnerDashboard.editActivity.settings");
  const [, start]        = useTransition();
  const [deleting, setDeleting]       = useState(false);
  const [statusPending, setStatusPending] = useState(false);
  const [currentStatus, setCurrentStatus] = useState<ActivityStatus>(activityStatus);
  ...
  function handleStatusToggle() {
    ...
    start(async () => {
      const res = await updateActivityStatus(activityId, nextStatus);
      if (res.success) {
  ...
  return (
    <div className="space-y-8">
      ...
      <AgentProgramSection
        ...
        agentCommissionEnabled={agentCommissionEnabled}
