...
function GuidniReviewSection({ guidniReview, transferId }: { guidniReview: GuidniReview; transferId: string }) {
  const t       = useTranslations("PartnerDashboard.editTransfer.reviews");
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {status && (
            <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full ${status.color}`}>
              {isPublished ? <FiCheckCircle className="h-3.5 w-3.5" /> : <FiClock className="h-3.5 w-3.5" />}
              {status.label}
            </span>
          )}
          <RequestReviewDialog
            ...
            offerHint={tDialog("transferOfferHint" as Parameters<typeof tDialog>[0])}
          />
        </div>
      </div>
        ...
        {editing ? (
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              ...
              {review.response && (
                <button type="button" onClick={() => { setReply(review.response!); setEditing(false); }} className="text-sm text-gray-400 hover:text-gray-600 transition-colors">
                  {t("guest.cancel")}
                </button>
              )}
            </div>
...
export function ReviewsTab({ reviews, guidniReview, transferId }: Props) {
  const t          = useTranslations("PartnerDashboard.editTransfer.reviews");
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white border border-gray-100 rounded-2xl p-6">
        <GuidniReviewSection guidniReview={guidniReview} transferId={transferId} />
      </div>
