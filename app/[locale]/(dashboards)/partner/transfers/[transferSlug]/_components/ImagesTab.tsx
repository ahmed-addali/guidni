...
import { setTransferImageAsCover, reorderTransferImages } from "@/lib/actions/partner-transfers";
...
export function ImagesTab({ transferId, images: initialImages }: Props) {
  ...
  function handleSetCover(imageId: string) {
    ...
    start(async () => {
      const res = await setTransferImageAsCover(imageId, transferId);
      if (res.success) {
        setImages((prev) => [
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-6 space-y-3">
        ...
        {cover ? (
          <div className="relative h-52 w-full sm:w-80 rounded-2xl overflow-hidden border border-gray-200 bg-gray-100">
            <Image src={cover.url} alt={t("coverTitle")} fill className="object-cover" sizes="320px" />
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-6 space-y-3">
        ...
        <ImageUploader
          ...
          images={images}
          onChange={setImages}
          onReorder={(orderedIds) => reorderTransferImages(transferId, orderedIds)}
        />
      </div>
    </div>
  );
