...
import { adminUpdateStayStatus, toggleStayFeatured } from "@/lib/actions/admin";
...
export function StayStatusActions({ id, status: initialStatus, featured: initialFeatured, labels }: Props) {
  ...
  function handleStatus(next: Status) {
    ...
    setStatus(next);
    start(async () => {
      const res = await adminUpdateStayStatus(id, next);
      if (!res.success) {
        setStatus(prev);
  ...
  function handleFeature() {
    ...
    setFeatured(next);
    start(async () => {
      const res = await toggleStayFeatured(id, next);
      if (!res.success) {
        setFeatured(!next);
