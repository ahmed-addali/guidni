...
import { STAY_CLIENT_SERVICE_FEE, TUNISIA_TVA_RATE } from "@/lib/constants/payments";
...
export function StayBookingCard({
  ...
  function handleBook() {
    if (!checkIn || !checkOut || belowMinStay || overCapacity) return;
    setLoading(true);
  ...
  return (
    <div className="border border-gray-200 rounded-2xl p-6 space-y-5 lg:sticky lg:top-8">
      ...
      {/* Date pickers */}
      <div className="grid grid-cols-2 border border-gray-200 rounded-xl overflow-hidden divide-x divide-gray-200">
        <div className="p-3">
      ...
      <div className="space-y-2 text-sm">
        <div className="flex justify-between text-gray-600">
          <span>{price} TND × {nights} {nightLabel}</span>
          <span>{baseTotal} TND</span>
        </div>

        {cleaning > 0 && (
          <div className="flex justify-between text-gray-600">
            <span>{labels.cleaningFee}</span>
            <span>{cleaning} TND</span>
          </div>
        )}
      ...
      <Button
        ...
        onClick={handleBook}
        disabled={loading || belowMinStay || overCapacity}
      >
